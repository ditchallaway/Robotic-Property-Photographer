...
function validateJob(job) {
    ...
    if (!job.boundaryRings || !isPolygonCoordinates(job.boundaryRings)) {
        errors.push("'boundaryRings' must be polygon coordinates [[[lon, lat], ...], ...]");
    }

    if (!job.boundaryOuter || !isLinearRing(job.boundaryOuter)) {
...
function normalizeJob(rawInput) {
    ...
    // Backward compatibility: flat outer ring only
        ...
        job.boundaryOuter = job.boundary;
        job.boundaryRings = [job.boundary];
        job.boundary = {
            type: 'Polygon',
            coordinates: [job.boundary]
        };
    }
